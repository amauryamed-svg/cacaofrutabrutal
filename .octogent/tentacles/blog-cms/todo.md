...
- [x] [P1] Wire `blog_read` token award in `BlogPost.tsx`: call award-tokens with `blog_read` event after 30s on page — once per slug per user per day — implemented 2026-06-22
- [ ] [P1] Wire `blog_share` token award in `BlogPost.tsx` share button (currently only wired in Ritual.tsx)
- [x] [P1] Build Markdown renderer for `body_md` field: use `marked` + `DOMPurify` for sanitized HTML — applied BRAND typography — implemented 2026-06-22
